...
Yelos 是一个持久性情感在场（persistent affective presence）**五幕表达逻辑**的参考实现，构建在 [`sylanne-core`] 情感引擎 SDK 之上。它是 AstrBot 插件 `astrbot_plugin_yelos` 的转世：同一个灵魂（`sylanne-core`，五幕情感逻辑），这一次被打包成了一个可导入的 Python 库，而不是一个消息管道插件。

> Embodiment 给了她一具身体，Yelos 给了她声带，与岁月。

具体来说，`yelos` 是一个 Python 包，可以直接 `import` 进宿主应用里（一个 agent 运行时、一个桌面陪伴软件，或任何已经拥有自己对话轮次循环、只想要情感/仲裁逻辑的东西）。它的工程形态是：
...
每一个可选的深化模块（`primal`/`arbiter`/`intrinsic`/`shadow`/`finitude` 中超出冻结核心的那部分组合逻辑）都**默认关闭**，并且在构造或运行时出错时会安全回退——产品不会因为某条深化路径失败而失去声音。
...
| 主动发言 / 衰老 | 关闭。 | 默认开启；`finitude_enabled=false` 或 `lifespan_active_days=0` 会换来一个不会衰老的永恒陪伴者。 |
...
没有什么会凭自己的意愿主动推送。所有她会在没被提示的情况下说出口的话——在压力下咽下某句话之后"重新涌上来"的延迟话语、一次主动的问候、一句梦呓、一个关切的轻推、当她的声音明显收窄时的一次纪元通知——都被缓冲在按会话划分的 **outbox** 里，带着到期时间和过期时间，只有在宿主调用 `manager.impulse(sid)` 时才会被取出。如果某个工具/方法的返回值里带着 `pending > 0`，或者 `arbitrate()` 返回了非 `None` 的 `delayed`，那就是该尽快去轮询 `impulse()` 的信号。如果始终没有人来轮询，缓冲的话语会过期，并被悄悄丢弃。
